...
//        }
    ...
    public SimulatedAnnealingState getNextState(int[] lastSingleCost) {
        int i;
...
//                	System.out.println("getNextState:"+i+" 重新来 "+tempY);
    ...
    /**
     * getNextState with LargestIndex
     * */
    public SimulatedAnnealingState getNextState2(int largeest) {
        int i;
        System.out.println("获取到的largeestIndex is "+ largeest);
        GridNode[] nextStateQueen = new GridNode[babynumber];
        //randomly pick a queen we want to change at a time.
        //用 rand来读取第随机个baby
        int rand = randomGenerator.nextInt(babynumber);
...
//                	System.out.println("getNextState:"+i+" 重新来 "+tempY);
                ...
                //new state will be added to the new state
                System.out.println("from "+q[i].x+","+q[i].y+"to "+tempX+","+tempY);
                nextStateQueen[i] = new GridNode(tempX, tempY);
                number[q[i].x]--;
                number[tempX]++;
            }
     ...
     * 
     * */
    public SimulatedAnnealingState getNextState3() {
        int i;
        GridNode[] nextStateQueen = new GridNode[babynumber];
        //randomly pick a queen we want to change at a time.
        //用 rand来读取第随机个baby
