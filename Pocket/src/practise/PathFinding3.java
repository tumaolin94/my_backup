...
//				System.out.println("  "+x+","+y+": "+matrix[x][y]);
				...
				for(int[] dir: dirs) {
					...
					if(new_x<0||new_x>=m||new_y<0||new_y>=n||matrix[new_x][new_y]=='0'||map.getOrDefault(new_position, -1)==new_key) continue;
					
					if(matrix[new_x][new_y]>='a'&&matrix[new_x][new_y]<='z') {
						new_key = tempKey|1<<(matrix[new_x][new_y]-'a');
					}
//					if(map.getOrDefault(new_position, -1)!=new_key) {
						map.put(new_position, new_key);
